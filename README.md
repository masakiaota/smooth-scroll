...
1. `/Applications/SmoothScroll.app`を開く。初回は準備画面の各「許可をリクエスト」を押し、macOSの案内に従って入力監視とアクセシビリティを許可する。アプリ自身が許可を要求するため、通常は「＋」からアプリを探す必要はない。確認画面が出ない場合は、同じボタンが「設定を開く」に変わるので、そこからSmoothScrollをオンにする。戻ると状態が更新され、両方が許可済みになったら「開始する」を押す。macOSから再起動を求められた場合は、その案内に従う。
...
送信中に対象アプリが変わる、除外対象になる、ネイティブ入力が入る場合は残量を破棄する。出力には現在のポインタ位置を設定し、システムのイベント経路へ送る。送信権限を確認できない場合は元の入力を通す。特殊なウィンドウ構成では平滑化されない場合がある。
